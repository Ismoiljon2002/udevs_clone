import Image from "next/image";
import { Col, Container, Nav, NavLink, Row } from "react-bootstrap";
import { FaInstagram, FaTwitter, FaYoutube } from "react-icons/fa";

function Footer() {

    return (
        <footer className="footer">
            <Container>
                <Image height={70} width={120} alt="Udevs" src="data:image/svg+xml;base64," />

                <Row>
                    <Col md={4}>
                        <Nav>
                            <h5>About us</h5>
                            <NavLink href="#direction">Direction</NavLink>
                            <NavLink href="#command">Command</NavLink>
                            <NavLink href="#tools">Tools</NavLink>
                            <NavLink href="#clients">Clients</NavLink>
                        </Nav>
                    </Col>
                    <Col md={5}>
                        <Nav>
                            <h5>Services</h5>
                            <NavLink href="#mobile">Development of mobile applications</NavLink>
                            <NavLink href="#erp">Development and implementation ERP systems</NavLink>
                            <NavLink href="#design">User interface, User experience design</NavLink>
                            <NavLink href="#consulting">IT consulting</NavLink>
                            <NavLink href="#optimization">Optimization IT consulting infrastructure</NavLink>
                        </Nav>
                    </Col>
                    <Col md={3}>
                        <Nav>
                            <h5>Portfolio</h5>
                            <NavLink href="#delever">Delever</NavLink>
                            <NavLink href="#smsuz">Sms.uz</NavLink>
                            <NavLink href="#goodzone">Goodzone</NavLink>
                            <NavLink href="#iman">Iman</NavLink>
                        </Nav>
                    </Col>

                </Row>

                <div className="copyright d-flex justify-content-between">
                    <p>© 2024 Udevs. All rights reserved</p>
                    <div className="social-media">
                        <NavLink href="https://udevs.io" > <FaInstagram /> </NavLink>
                        <NavLink href="https://udevs.io" > <FaTwitter /> </NavLink>
                        <NavLink href="https://udevs.io" > <FaYoutube /> </NavLink>
                    </div>
                </div>

            </Container>
        </footer>
    );
}

export default Footer;